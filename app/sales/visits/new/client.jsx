...
export default function SalesNewVisitClient({ user, preselectedCpId }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await fetch(`/api/sales/cps/${preselectedCpId}`);
...
function CpPicker({ onSelect }) {
  ...
  return (
    <div>
      <Link href="/sales" className="sx-back">
      ...
      <Link href="/sales/cps/new" className="oh-btn ghost block" style={{ marginBottom: 14 }}>
