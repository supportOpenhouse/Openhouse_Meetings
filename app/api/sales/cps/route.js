...
// POST /api/sales/cps — register a NEW channel partner in Open House Core via
...
export async function POST(request) {
  ...
  // Core attributes the broker to a sales manager. Until each rep is mapped to a
  // Core sales_manager_id, fall back to the configured default.
  const salesManagerId = Number(
    session.user.salesManagerId || process.env.CP_MEETINGS_DEFAULT_SALES_MANAGER_ID
  );
  if (!Number.isFinite(salesManagerId)) {
    return NextResponse.json(
      { error: 'Sales manager is not configured. Set CP_MEETINGS_DEFAULT_SALES_MANAGER_ID.' },
      { status: 503 }
    );
  }
