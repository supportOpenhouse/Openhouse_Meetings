...
// (/api/supply/cps POST → CP Meetings broker API). The API key + sales_manager_id
...
export default function SalesCpNewClient() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await fetch('/api/supply/cps/cities');
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await fetch(`/api/supply/cps/micro-markets?city=${encodeURIComponent(city)}`);
        const data = await res.json();
  ...
  async function submit(e) {
    ...
    setSaving(true);
    try {
      const res = await fetch('/api/supply/cps', {
        method: 'POST',
  ...
  return (
    <div>
      {!done && (
        <Link href="/supply/cps" className="reg-back">
      ...
      {!configured ? (
        <div className="sx-empty" style={{ marginTop: 22, padding: '40px 24px' }}>
          ...
          <Link href="/supply/cps" className="oh-btn accent" style={{ marginTop: 14 }}>
            Browse partners
          </Link>
        </div>
      ) : done ? (
        <div className="sx-empty" style={{ marginTop: 22, padding: '40px 24px' }}>
          ...
          <div className="reg-actions">
            {done.cp_code && (
              <Link href={`/supply/cps/${encodeURIComponent(done.cp_code)}`} className="oh-btn accent">
                View partner
              </Link>
            )}
            ...
            <Link href="/supply/cps" className="oh-btn ghost">
              Done
            </Link>
          </div>
        </div>
