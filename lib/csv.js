...
// A Google Maps link for a captured lat/lng (blank if the visit had no location).
export function mapsLink(lat, lng) {
  if (lat === null || lat === undefined || lat === '' || lng === null || lng === undefined || lng === '') return '';
...
const MEETING_BASE_HEADERS = [
  'Meeting Date', 'CP Code', 'CP Name', 'CP Number', 'City', 'Latitude', 'Longitude', 'Map',
  'RM', 'RM Email', 'Duration', 'Duration (s)', 'Meeting Type', 'Status', 'Sentiment', 'Lead Score',
...
export function meetingSummaryFields(s, type) {
  const sub = (s && s[type]) || s?.engagement || s?.visit || s?.call || s?.onboarding || s?.negotiation || {};
  const pick = (...keys) => {
    for (const k of keys) {
      const v = sub[k];
  ...
  return {
    ...
    score: s?.score?.total ?? '',
    keyTopics: pick('key_topics', 'open_feedback', 'persona_summary', 'cp_interest_signals', 'discussion_summary'),
    requirements: pick('cp_requirements', 'configuration', 'size_range', 'location_priorities', 'cp_profile', 'buyer_intent'),
    properties: pick('properties_discussed', 'societies_of_interest', 'societies_explored', 'most_liked_society'),
    budget: pick('budget', 'budget_range', 'price_discussion', 'payment_plan'),
    objections: pick('objections', 'objections_raised', 'frustrations'),
    commitments: pick('commitments', 'commitments_made', 'agreed_terms', 'concessions_offered'),
    nextAction: pick('next_action', 'sales_recommendation'),
    followUp: pick('follow_up_date', 'revisit_plan', 'closure_timeline', 'close_timeline'),
  };
}
...
export function meetingCsvRow(m, { funnel = false } = {}) {
  ...
  const base = [
    ...
    m.location_lat ?? '', m.location_lng ?? '', mapsLink(m.location_lat, m.location_lng),
...
const VISIT_HEADERS = [
  'Visit Date', 'CP Code', 'CP Name', 'CP Number', 'City', 'Latitude', 'Longitude', 'Map',
  'RM', 'Duration', 'Duration (s)', 'Visit Type', 'Outcome', 'Engagement', 'Onboarding Stage',
...
export function visitCsvRow(v) {
  ...
  return [
    ...
    v.meeting_lat ?? '', v.meeting_lng ?? '', mapsLink(v.meeting_lat, v.meeting_lng),
