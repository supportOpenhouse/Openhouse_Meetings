...
const SELECT_COLS = {
  ...
  created_at: channelPartners.created_at,
  city: cities.name,
};
...
// Tag every inventory cache entry so registering a new partner can invalidate
// them all at once — otherwise a brand-new CP stays invisible for INVENTORY_TTL.
export const INVENTORY_TAG = 'inv-cp';
...
// opts.fresh skips the cache entirely — used by the "Refresh" button so a
// just-registered partner shows up without waiting out INVENTORY_TTL.
export async function searchInventoryCps(query, limit = 30, opts = {}) {
  if (!cpDb) return { configured: false, cps: [] };
  if (opts.fresh) return runSearchInventoryCps(query, limit);
  return unstable_cache(
    () => runSearchInventoryCps(query, limit),
    ['inv-cp-search', (query || '').trim().toLowerCase(), String(limit)],
    { revalidate: INVENTORY_TTL, tags: [INVENTORY_TAG] }
  )();
}

async function runRecentlyAddedCps(limit) {
  const rows = await cpDb
    .select(SELECT_COLS)
    .from(channelPartners)
    .leftJoin(cities, eq(cities.id, channelPartners.city_id))
    .where(sql`${channelPartners.created_at} is not null`)
    .orderBy(desc(channelPartners.created_at))
    .limit(limit);
  return { configured: true, cps: rows.map(mapCp) };
}

// The most recently registered partners across the shared inventory — so a
// partner just added (by anyone; the external table doesn't attribute a rep)
// is visible on the CP page without searching. Cached + tagged, so registering
// a new partner invalidates it (revalidateTag(INVENTORY_TAG)).
export async function recentlyAddedCps(limit = 12, opts = {}) {
  if (!cpDb) return { configured: false, cps: [] };
  if (opts.fresh) return runRecentlyAddedCps(limit);
  return unstable_cache(
    () => runRecentlyAddedCps(limit),
    ['inv-cp-recent', String(limit)],
    { revalidate: INVENTORY_TTL, tags: [INVENTORY_TAG] }
  )();
}
