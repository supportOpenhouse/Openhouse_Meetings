...
// Extracts just the temperature/sentiment string from the summary jsonb in SQL,
// so list endpoints don't ship the full multi-KB summary object to render a
// single pill. Path precedence mirrors getSentiment() in MeetingsTable. Detail
// views (getMeetingById) still return the full summary.
function listSentiment() {
  return sql`coalesce(
    ${meetings.summary} #>> '{score,classification}',
    ${meetings.summary} #>> '{engagement,sentiment}',
    ${meetings.summary} #>> '{onboarding,sentiment}',
    ${meetings.summary} #>> '{call,sentiment}',
    ${meetings.summary} #>> '{sentiment}'
  )`;
}
...
export async function listAllMeetings({ rmFilter, since, until, city, search, includeTranscript = false, includeSummary = false } = {}) {
  ...
  // List views only need `list_sentiment` (lean). Consumers that render the full
  // AI summary — e.g. the admin CSV export — opt back into the heavy jsonb here.
  if (includeSummary) {
    columns.summary = meetings.summary;
  }
