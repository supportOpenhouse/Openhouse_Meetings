...
export async function createSalesVisit({ visit, audioUrl, durSec } = {}) {
  ...
  const createRes = await fetch('/api/supply/visits', {
    method: 'POST',
  ...
  // Fire-and-forget — .catch swallows a dropped network (mobile) so it doesn't
  // surface as an unhandled rejection in the error log. The row exists already.
  fetch(`/api/supply/visits/${visitId}/process`, { method: 'POST', keepalive: true }).catch(() => {});
