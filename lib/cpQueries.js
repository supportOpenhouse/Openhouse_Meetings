...
import { cpDb, isCpDbConfigured, channelPartners, normalizePhone } from './cpDb';
...
export async function getCpDashboardData({ rmId = null, monthsToShow = 4, today = new Date() }) {
  ...
  // Opportunistically fetch CP names + phones from the external inventory DB
  // so the dashboard can show / search by them. If cpDb isn't configured we
  // just leave them null and the UI degrades gracefully.
  const cpInfo = await fetchCpInfo(assignments.map((a) => a.cp_code));
  ...
  const cps = assignments.map((a) => {
    ...
    const info = cpInfo.get(a.cp_code) || null;
    return {
      cp_code: a.cp_code,
      cp_name: info?.name || null,
      cp_phone: info?.phone || null,
      cp_phone_normalized: info?.phone ? normalizePhone(info.phone) : null,
      cp_company: info?.company || null,
      rm_id: a.rm_id,
      rm_name: a.rm_name,
      rm_email: a.rm_email,
...
// Returns Map<cp_code, { name, phone, company }> by querying the external CP
// inventory DB in one shot. Returns an empty Map if cpDb isn't configured.
async function fetchCpInfo(cpCodes) {
  const out = new Map();
  if (!isCpDbConfigured() || cpCodes.length === 0) return out;
  try {
    const rows = await cpDb
      ...
      .where(inArray(channelPartners.cp_code, cpCodes));
    for (const r of rows) out.set(r.cp_code, r);
  } catch (e) {
    console.warn('[cp-dashboard] CP inventory lookup failed:', e?.message || e);
  }
  return out;
}
