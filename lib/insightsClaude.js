...
//
// `cpCodes` !== null → scope the corpus to meetings for those CP codes (used
// to give an RM a view of only their assigned CPs).
async function loadCorpus(corpusType, periodDays, cpCodes = null) {
  const since =
  ...
  if (since && scoped) {
    rows = await sql`
      ...
      WHERE meeting_type = ANY(${typeFilter}) AND status = 'ready' AND summary IS NOT NULL
        AND started_at >= ${since} AND cp_code = ANY(${cpCodes}::text[])
      ORDER BY started_at DESC LIMIT ${MAX_MEETINGS}`;
  } else if (since && !scoped) {
    rows = await sql`
      SELECT id, meeting_type, cp_code, cp_name, started_at, summary
      FROM meetings
      WHERE meeting_type = ANY(${typeFilter}) AND status = 'ready' AND summary IS NOT NULL
...
// rmId !== null → scope the corpus to that RM's assigned CPs.
export async function generateStandardInsight(insightKey, periodDays = 90, rmId = null) {
  const def = STANDARD_INSIGHTS[insightKey];
  if (!def) throw new Error(`Unknown insight: ${insightKey}`);

  let cpCodes = null;
  if (rmId) {
    const assigned = await sql`SELECT cp_code FROM cp_assignments WHERE rm_id = ${rmId}`;
    cpCodes = assigned.map((r) => r.cp_code);
  }

  const { list, refMap } = await loadCorpus(def.corpus, periodDays, cpCodes);
  if (list.length === 0) {
    return {
      meetingCount: 0,
      result: {
