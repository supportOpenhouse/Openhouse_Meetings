...
export async function uploadAndCreateMeeting({
  ...
  const newBlob = await upload(filename, blob, {
    ...
    // `addRandomSuffix` is authorised server-side in /api/upload-url so
    // IndexedDB-queue retries don't collide with a partially-committed
    // earlier attempt at the same deterministic filename.
    onUploadProgress: (e) => {
  ...
  const createRes = await fetch('/api/meetings/create', {
    ...
    body: JSON.stringify({
      ...
      // Visit dropdown linkage — only set when the meeting was started by
      // picking a scheduled visit from the Google Sheet (see /new-meeting).
      cp_visit_id: form.cp_visit_id ?? null,
      cp_visit_meta: form.cp_visit_meta ?? null,
    }),
    keepalive: true,
  });
  if (!createRes.ok) {
