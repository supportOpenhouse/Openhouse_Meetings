...
async function callField(key, since) {
...
async function callArrayField(key, since) {
  const rows = await sql`
...
export async function getCallMetrics(periodDays = 90) {
  const since = sinceClause(periodDays);
  ...
  const [journeyStage, timeline, budget, configuration, blockers, frustrations, volumeByWeek, topRms] =
    await Promise.all([
      callField('journey_stage', since),
      callField('move_in_timeline', since),
      callField('budget', since),
      callArrayField('configuration', since),
      callField('booking_blocker', since),
      callArrayField('frustrations', since),
      weeklyVolume('call', periodDays),
      topRmsByVolume('call', periodDays),
    ]);
