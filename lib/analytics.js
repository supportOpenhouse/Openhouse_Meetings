...
// Negotiation (deal-closing) meetings — scored on the negotiation rubric.
const NEGOTIATION_FUNNEL_PARAMS = [
  ['substantial_discussion', '15+ min negotiation'],
  ['price_negotiated', 'Price negotiated'],
  ['price_aligned', 'Price aligned / near agreement'],
  ['payment_plan_discussed', 'Payment plan worked out'],
  ['decision_maker_present', 'Decision-maker present'],
  ['concrete_timeline', 'Concrete close timeline'],
  ['token_or_booking_intent', 'Token / booking intent'],
  ['objections_resolved', 'Objections resolved'],
];

export async function getNegotiationMetrics(periodDays = 90, opts = {}) {
  const { since, until, rmId } = effectiveFilters(periodDays, opts);
  const [r] = await sql`
    SELECT
      count(*)::int AS total,
      ...
      count(*) FILTER (WHERE summary->'score'->>'classification' = 'cold')::int AS cold,
      round(avg((summary->'score'->>'total')::numeric), 1) AS avg_score,
      count(*) FILTER (WHERE summary->'score'->'parameters'->'substantial_discussion'->>'met' = 'true')::int AS p_substantial_discussion,
      count(*) FILTER (WHERE summary->'score'->'parameters'->'price_negotiated'->>'met' = 'true')::int AS p_price_negotiated,
      count(*) FILTER (WHERE summary->'score'->'parameters'->'price_aligned'->>'met' = 'true')::int AS p_price_aligned,
      count(*) FILTER (WHERE summary->'score'->'parameters'->'payment_plan_discussed'->>'met' = 'true')::int AS p_payment_plan_discussed,
      count(*) FILTER (WHERE summary->'score'->'parameters'->'decision_maker_present'->>'met' = 'true')::int AS p_decision_maker_present,
      count(*) FILTER (WHERE summary->'score'->'parameters'->'concrete_timeline'->>'met' = 'true')::int AS p_concrete_timeline,
      count(*) FILTER (WHERE summary->'score'->'parameters'->'token_or_booking_intent'->>'met' = 'true')::int AS p_token_or_booking_intent,
      count(*) FILTER (WHERE summary->'score'->'parameters'->'objections_resolved'->>'met' = 'true')::int AS p_objections_resolved
    FROM meetings
    WHERE meeting_type = 'negotiation' AND status = 'ready' AND summary IS NOT NULL
      AND (${since}::timestamptz IS NULL OR started_at >= ${since}::timestamptz)
      AND (${until}::timestamptz IS NULL OR started_at <= ${until}::timestamptz)
      AND (${rmId}::uuid IS NULL OR rm_id = ${rmId}::uuid)
  `;

  const funnel = NEGOTIATION_FUNNEL_PARAMS.map(([key, label]) => {
    const met = r[`p_${key}`] || 0;
    return { key, label, met, total: r.total, pct: r.total ? Math.round((met / r.total) * 100) : 0 };
  });
  ...
  return {
    ...
    byClassification: { hot: r.hot, warm: r.warm, cold: r.cold },
    avgScore: r.avg_score != null ? Number(r.avg_score) : null,
    closing: r.p_token_or_booking_intent,
    funnel,
    volumeByWeek: await weeklyVolume('negotiation', periodDays, opts),
    topRms: await topRmsByScore(periodDays, opts, 'negotiation'),
  };
}
...
async function topRmsByScore(periodDays, opts = {}, meetingType = 'visit') {
  const { since, until, rmId } = effectiveFilters(periodDays, opts);
  return sql`
    ...
    WHERE m.meeting_type = ${meetingType} AND m.status = 'ready' AND m.summary IS NOT NULL
