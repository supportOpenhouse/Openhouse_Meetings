...
export function isSalestrailConfigured() {
  return !!(process.env.SALESTRAIL_API_USERNAME && process.env.SALESTRAIL_API_PASSWORD);
}

function authHeader() {
  const u = process.env.SALESTRAIL_API_USERNAME || '';
  const p = process.env.SALESTRAIL_API_PASSWORD || '';
...
export async function listCallsByCreated(fromDate, toDate) {
  ...
  if (!res.ok) {
    ...
    throw new Error(`Salestrail list failed (${res.status}): ${body.slice(0, 200)}`);
  }
