...
async function loadCorpus(periodDays, opts = {}) {
  ...
  `;
  // Total-size budget so a large slice can't push the prompt past the 200k
  // context window. Hindi/Hinglish transcripts tokenize densely (~2.2 chars/
  // token), so 300k chars ≈ 135k tokens of data — safely under 200k.
  ...
  for (const r of rows) {
    ...
    if (list.length > 0 && used + size > CORPUS_CHAR_BUDGET) break;
    used += size;
    refMap[ref] = r.id;
    list.push(entry);
  }
